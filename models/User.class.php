...
Class User
{
    ...
    public function validate($data)
    {
        ...
        if (isset($data['pseudo'])) {
            if (empty($data['pseudo'])) {
                ...
                // si name > 50 chars
            } else if (mb_strlen($data['pseudo']) < 2) {
                $this->errors[] = 'champ pseudo trop court (8 min)';
            } else if (mb_strlen($data['pseudo']) > 45) {
                $this->errors[] = 'champ pseudo trop long (20 max)';
            }
        }
    ...
    public function save($data)
    {
        if ($this->validate($data)) {
            ...
            $dbh = Connection::get();
            $sql = "insert into users (login, password, pseudo) values (:login, :password , :pseudo)";
            $sth = $dbh->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
